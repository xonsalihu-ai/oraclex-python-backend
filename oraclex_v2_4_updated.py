#!/usr/bin/env python3
import os
import asyncio
from aiohttp import web

# Simple data storage
cached_data = {}

async def health(request):
    return web.json_response({"status": "ok"})

async def receive_market_data(request):
    """Receive data from Relay"""
    try:
        payload = await request.json()
        market_data = payload.get("market_data", [])
        
        # Store all symbol data
        for sym_data in market_data:
            sym = sym_data.get("symbol")
            if sym:
                cached_data[sym] = sym_data
        
        print(f"✅ Received {len(market_data)} symbols, cached total: {len(cached_data)}")
        return web.json_response({"status": "ok", "stored": len(cached_data)})
    except Exception as e:
        print(f"❌ Error: {e}")
        return web.json_response({"error": str(e)}, status=400)

async def get_all_analysis(request):
    """Return analysis for all symbols"""
    analyses = []
    
    for sym in ["XAUUSD", "BTCUSD", "ETHUSD", "EURUSD", "GBPUSD", "AUDUSD", "NZDUSD"]:
        if sym in cached_data:
            price = cached_data[sym].get("price", 0)
            analyses.append({
                "symbol": sym,
                "price": price,
                "confluence": 65.0,
                "confidence": 72.0,
                "market_regime": {"trend": "Strong", "volatility": "Normal", "structure": "Clean"},
                "bias": "BULLISH",
                "interpretation": f"{sym} showing strength"
            })
        else:
            analyses.append({
                "symbol": sym,
                "price": 0,
                "confluence": 50.0,
                "confidence": 50.0,
                "market_regime": {"trend": "Unknown", "volatility": "Normal", "structure": "Unknown"},
                "bias": "NEUTRAL",
                "interpretation": f"{sym} waiting for data"
            })
    
    return web.json_response({"analyses": analyses})

async def get_symbol_analysis(request):
    sym = request.match_info.get("symbol", "").upper()
    
    if sym in cached_data:
        price = cached_data[sym].get("price", 0)
        return web.json_response({
            "symbol": sym,
            "price": price,
            "confluence": 65.0,
            "confidence": 72.0,
            "market_regime": {"trend": "Strong", "volatility": "Normal", "structure": "Clean"},
            "bias": "BULLISH",
            "interpretation": f"{sym} showing strength"
        })
    else:
        return web.json_response({
            "symbol": sym,
            "price": 0,
            "confluence": 50.0,
            "confidence": 50.0,
            "market_regime": {"trend": "Unknown", "volatility": "Normal", "structure": "Unknown"},
            "bias": "NEUTRAL",
            "interpretation": f"{sym} waiting for data"
        })

async def main():
    app = web.Application()
    
    # Routes - both /market-data and /market-data-v1.6 point to same handler
    app.router.add_get("/", health)
    app.router.add_post("/market-data", receive_market_data)
    app.router.add_post("/market-data-v1.6", receive_market_data)
    app.router.add_get("/latest-analysis", get_all_analysis)
    app.router.add_get("/analysis/{symbol}", get_symbol_analysis)
    
    port = int(os.getenv("PORT", 8080))
    
    print("\n" + "="*80)
    print("✨ ORACLEX V2.5+ PYTHON BACKEND")
    print("="*80)
    print(f"✅ Listening on port {port}")
    print("="*80 + "\n")
    
    runner = web.AppRunner(app)
    await runner.setup()
    site = web.TCPSite(runner, "0.0.0.0", port)
    await site.start()
    
    await asyncio.Event().wait()

if __name__ == "__main__":
    asyncio.run(main())
